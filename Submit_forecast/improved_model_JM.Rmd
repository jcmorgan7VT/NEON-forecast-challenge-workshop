...
targets_use <- targets |> 
  dplyr::filter(site_id == site,
                variable == var,
                datetime < start)  |> 
  right_join(noaa_past_mean, by = join_by(datetime)) #|> 
  #tsibble::as_tsibble(key = c('variable', 'site_id'), index = 'datetime') |>
  #tsibble::fill_gaps()   # add NA values up to today (index)
...
forecast_starts <- noaa_past_mean |> 
   |> 
  left_join(targets_use, by = join_by(datetime)) |> 
  tsibble::as_tsibble(key = c('variable', 'site_id'), index = 'datetime')
##########
...
#set up what model to use- example is RW, for random walk
RW_model <- forecast_starts |> 
  fabletools::model(RW = fable::RW(observation ~ air_pressure + air_temperature + eastward_wind + northward_wind + precipitation_flux + relative_humidity + surface_downwelling_longwave_flux_in_air + surface_downwelling_shortwave_flux_in_air + drift())) 
